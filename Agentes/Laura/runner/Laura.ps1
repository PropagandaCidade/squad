...
Write-Host "Agente: Laura" -ForegroundColor Cyan
Write-Host "Especialidade: Customer Success" -ForegroundColor Yellow
...
if (Get-Command Update-AgentHeartbeat -ErrorAction SilentlyContinue) {
    Update-AgentHeartbeat -AgentName "Laura" -TaskId $TaskId -Task $Task -Status "in_progress" -Note "Runner iniciado" | Out-Null
}
...
if (Test-Path -LiteralPath $GamificationScript) {
    ...
    if (Get-Command Register-AgentAction -ErrorAction SilentlyContinue) {
        if ($Points -ge 0) {
            $g = Add-AgentPoints -AgentName 'Laura' -Points $Points -Task $Task -Badges @('Execucao Registrada')
        } else {
            $g = Register-AgentAction -AgentName 'Laura' -Task $Task -Category 'execution' -Complexity $Complexity -Outcome $Outcome -Badges @('Execucao Registrada')
        }
...
if (Get-Command Update-AgentHeartbeat -ErrorAction SilentlyContinue) {
    ...
    Update-AgentHeartbeat -AgentName "Laura" -TaskId $TaskId -Task $Task -Status $heartbeatStatus -Outcome $Outcome -Note "Runner finalizado" | Out-Null
}
