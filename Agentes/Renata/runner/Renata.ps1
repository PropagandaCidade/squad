...
Write-Host "Agente: Renata" -ForegroundColor Cyan
Write-Host "Especialidade: HR Manager" -ForegroundColor Yellow
...
if (Get-Command Update-AgentHeartbeat -ErrorAction SilentlyContinue) {
    Update-AgentHeartbeat -AgentName "Renata" -TaskId $TaskId -Task $Task -Status "in_progress" -Note "Runner iniciado" | Out-Null
}
...
if (Test-Path -LiteralPath $GamificationScript) {
    ...
    if (Get-Command Register-AgentAction -ErrorAction SilentlyContinue) {
        if ($Points -ge 0) {
            $g = Add-AgentPoints -AgentName 'Renata' -Points $Points -Task $Task -Badges @('Execucao Registrada')
        } else {
            $g = Register-AgentAction -AgentName 'Renata' -Task $Task -Category 'execution' -Complexity $Complexity -Outcome $Outcome -Badges @('Execucao Registrada')
        }
...
if (Get-Command Update-AgentHeartbeat -ErrorAction SilentlyContinue) {
    ...
    Update-AgentHeartbeat -AgentName "Renata" -TaskId $TaskId -Task $Task -Status $heartbeatStatus -Outcome $Outcome -Note "Runner finalizado" | Out-Null
}
