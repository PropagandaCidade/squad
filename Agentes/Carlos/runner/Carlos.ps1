...
Write-Host "Agente: Carlos" -ForegroundColor Cyan
Write-Host "Especialidade: Produtor Musical" -ForegroundColor Yellow
...
if (Get-Command Update-AgentHeartbeat -ErrorAction SilentlyContinue) {
    Update-AgentHeartbeat -AgentName "Carlos" -TaskId $TaskId -Task $Task -Status "in_progress" -Note "Runner iniciado" | Out-Null
}
...
if (Test-Path -LiteralPath $GamificationScript) {
    ...
    if (Get-Command Register-AgentAction -ErrorAction SilentlyContinue) {
        if ($Points -ge 0) {
            $g = Add-AgentPoints -AgentName "Carlos" -Points $Points -Task $Task -Badges @("Audio em Acao")
        } else {
            $g = Register-AgentAction -AgentName "Carlos" -Task $Task -Category "audio_production" -Complexity $Complexity -Outcome $Outcome -Badges @("Audio em Acao")
        }
...
if (Get-Command Update-AgentHeartbeat -ErrorAction SilentlyContinue) {
    ...
    Update-AgentHeartbeat -AgentName "Carlos" -TaskId $TaskId -Task $Task -Status $heartbeatStatus -Outcome $Outcome -Note "Runner finalizado" | Out-Null
}
